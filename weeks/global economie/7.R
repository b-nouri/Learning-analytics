...
course1_7$test7 <- replace(course1_7$test7, is.na(course1_7$test7), 0)
...
course1_7$test_percentage7 <- replace(course1_7$test_percentage7, is.na(course1_7$test_percentage7), 0)
...
###------week7 dataset#-----
weeks7 <- course[,47:53]
sum_weeks7 <- course[,65:70]
days_week7 <- course[,82:88]

video_week7 <- course[,100:134] #8 videos
course1_7 <- course1_7 %>%
...
train_course1 <- train1[,5:76]
train_course1 <- bind_cols(train_course1, scores)

id_test1 <- test1[,1:2]
test_course1 <- test1[,5:76]
...
gbm_tune7 <- train(score_class ~ ., data = train_course1,
                   method = "gbm",
                   metric = "ROC",
                   trControl = ctrl,
                   tuneGrid=grid,
...
gbm_fit7 <- train(score_class ~ ., data = train_course1,
                   method = "gbm",
                   metric = "ROC",
                   trControl = ctrl,
                   tuneGrid=grid1,
...
res <- gbm_fit7$results
res

gbm_pred7 <- predict(gbm_fit7,test_course1)

confusionMatrix(gbm_pred7,test7$score_class)   

week7_prob <- predict(gbm_fit7,test_course1,type="prob")
